...
class Servicio extends Model
{
    use HasFactory;
     ...
     private function btnEdit(int $id):string {
        ...
        return '<a href="'.route("servicios.edit",$id).'" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar">
    ...
    private function btnDelete(int $id):string {
        return '
        <form action="'.route('servicios.destroy',$id).'" method="post" class="d-inline">'.
    ...
    protected function getRowAttribute($value){
        return [$this->id,$this->plantillaUsuario->nombre,$this->nombre,$this->descripcion,$this->icono,'<nobr>'.$this->btnEdit($this->id).$this->btnDelete($this->id).$this->btnDetails($this->plantilla_usuario_id).'</nobr>'];
    }
